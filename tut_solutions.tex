...
% One can show by induction that such a function, if it exists, must be unique. Let $+$ and $+'$ be two candidate functions that satisfy these conditions. We have $\forall a \in \mathbb{N}, a + 0 = a +' 0$ (both equal $a$). For any $b \in \mathbb{N}$, if $a + b = a +' b$ (say both equal $k$), then $a +' next(b) = a +' next(b) = next(k)$. Therefore, $\forall a, b \in \mathbb{N}, a + b = a +' b$, ie $+ = +'$.
